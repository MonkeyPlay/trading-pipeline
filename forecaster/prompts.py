...
"""
Prompt template generator for the NQ Opening Forecast System.
...
def construct_forecast_prompt(target_day, target_features, analogues):
    """
...
- Hist Volatility:    {_fmt(tf.get('historical_volatility'), '.4f')}
=====================================================
"""
...
Pre-Open Context:
  ...
  - Overnight Range: {_fmt(snap.get('overnight_range'))} | Hist Vol: {_fmt(snap.get('historical_volatility'), '.4f')}
...
"""
    ...
    prompt = f"""You are an elite quantitative microstructural analyst specializing in the Nasdaq-100 (NQ) index futures opening session.

Your task is to analyze the opening context for the target session {target_day} by evaluating its pre-open metrics against the provided historical analogue days and their realized outcomes.
